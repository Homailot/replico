...
\section{Networking}
    ...
    \subsection{User Manager} \label{sec:user_manager}
        ...
        The user manager is responsible for keeping track of connected users. It stores the association of user IDs to Netcode client IDs, the current point of interest ID, and the connected user IDs. When a user connects, the user manager assigns a user ID and updates the connected user IDs. For this prototype, only two users can connect at a time. Therefore, the user manager has a list of available user IDs that it assigns to new users, 0 and 1. After updating the user's ID network variable, it requests the table manager to add the player to an available table, as described in Section \ref{sec:table_manager}. When a user disconnects, the user manager removes the user ID from the connected user IDs and adds it back to the available user IDs. Then, it requests the table manager to remove the player from their table.
    ...
    % data stored on the server:
        ...
        % Events
            % when a player joins (user manager)
                % move player to available table
                % assign player id
            % when a player leaves (user manager)
        ...
        % Messages (RPCS)
            ...
            % client -> server:
                % move player in table to position, used when teleporting
                ...
                % move player to table, used when joining a table
                    % used when joining the table
                        % will destroy the origin table network object if it has no players
                % create point of interest
                    % increment and get point of interest id
                    % used when creating a point of interest
                % remove point of interest
                    % used when removing a point of interest
            % server -> client:
                % move player to table client rpc
                    % used when moving a player to a table
                    % asks the client to move its player to the table, updatin the posititon
